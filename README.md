...
1. ~~`idf.py menuconfig`でRequest Header 拡張~~ -> ESP-IDF v6.0.0ではデフォルトで1024になっていた  
...
## Ploty.js
自作のグラフ描画スクリプト
使用方法は /ploty/src/ploty.js などを参照  
- 簡単な2Dプロットおよび3Dプロットを描画可能
- 必要時にESPからクライアントに配信 
    - ESPではバイナリファイルを以下のようにROM領域に埋め込むことができる (cf.[ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-guides/build-system.html#embedding-binary-data))
      ```CMake
      # components\wifi_ui\CMakeLists.txt
      target_add_binary_data(${COMPONENT_LIB} "${CMAKE_SOURCE_DIR}/path/to/script.min.js.gz" BINARY)
      ```
      ```c
      extern const uint8_t script_min_js_gz_start[] asm("_binary_script_min_js_gz_start");
      extern const uint8_t script_min_js_gz_end[]   asm("_binary_script_min_js_gz_end");
      const size_t len = script_min_js_gz_end - script_min_js_gz_start;
      ```
